...
public class WatchServiceSubscription {
    ...
    public WatchServiceSubscription(String eventToMonitor, String directory, String pollingInterval,
                                    Consumer<WatchServiceSubscriptionEvent> callback) {
                ...
                if (watchKey != null) {
                    for (WatchEvent<?> event : watchKey.pollEvents()) {
                        ...
                        if (event.kind().toString().equals(eventToMonitor)) {
                            WatchServiceSubscriptionEvent wsse = new WatchServiceSubscriptionEvent(eventToMonitor,
                                    directory, event.context().toString());
